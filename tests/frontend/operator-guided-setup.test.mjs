...
test("Review and Launch uses the shared task-aware mutation dispatcher", async () => {
  ...
  assert.match(main, /dispatchTaskAwareLaunch\(guidedLaunch, state\.activeStage\)/);
  assert.match(main, /dispatchTaskAwareLaunch\("workflow"\)/);
  assert.match(main, /dispatchTaskAwareLaunch\("stage", state\.activeStage\)/);
});
