...
def _grant() -> RepairExtensionGrant:
    return RepairExtensionGrant(
        ...
        author="operator", authorized_at_utc="2026-09-06T00:00:00Z", reason="One correction",
    )


def _metadata() -> StageRunMetadata:
    return StageRunMetadata.create(
        run_id="run-1", work_item_id="WI-1", stage="plan", status="pending",
...
def _ledger() -> TaskLedger:
    return TaskLedger(
        ...
        created_at_utc="2026-09-06T00:00:00Z", updated_at_utc="2026-09-06T00:00:00Z",
    )
...
def test_current_records_reject_missing_or_noncurrent_schema(
    current_record: tuple[dict[str, Any], Callable[..., Any]], version: object,
) -> None:
...
@pytest.mark.parametrize("field,value", (
    ("status_history", []), ("status_history", [None]),
    ("status_history", [{"status": "pending"}]),
    ("status_history", [{"status": None, "changed_at_utc": "now"}]),
    ("repair_history", ["repair"]), ("repair_history", None),
    ("repair_extension_grant", {}), ("repair_extension_grant", False),
    ("run_id", None), ("created_at_utc", 1),
))
...
@pytest.mark.parametrize("field,value", (
    ("attempt_number", "1"), ("attempt_number", True), ("trigger", None),
    ("outcome", False), ("recorded_at_utc", None), ("validator_report_path", 4),
))
def test_repair_history_requires_recorded_types_and_nullable_path_keys(
    field: str, value: object,
) -> None:
    entry = RepairHistoryEntry(
        attempt_number=1, trigger="repair", outcome="failed", recorded_at_utc="now",
    ).to_dict()
    entry[field] = value
...
@pytest.mark.parametrize("field,value", (
    ("tasks", [None]), ("tasks", {}), ("finalization", None), ("finalization", {}),
    ("created_at_utc", None), ("source_tasklist_sha256", 1),
))
...
@pytest.mark.parametrize("field,value", (
    ("task_id", None), ("status", [None]), ("files", {"path": None}),
))
