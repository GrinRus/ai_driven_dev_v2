...
def test_build_native_subprocess_spec_uses_prompt_file_without_adapter_flags(
    tmp_path: Path,
) -> None:
    context = _context(tmp_path)
    ...
    spec = build_subprocess_spec(
        configured_command=get_runtime_definition("opencode").default_command,
        context=context,
        base_env={"BASE_FLAG": "1"},
        repository_root=tmp_path,
    ...
    assert spec.command[:4] == ("opencode", "run", "--format", "json")
...
def test_run_subprocess_with_streaming_completes_for_unanswered_questions(
    tmp_path: Path,
) -> None:
    stage_root = tmp_path / ".aidd" / "workitems" / "WI-123" / "stages" / "idea"
    stage_root.mkdir(parents=True)
    idea_brief = stage_root / "idea-brief.md"
    questions = stage_root / "questions.md"
    answers = stage_root / "answers.md"
    stage_result = stage_root / "stage-result.md"
    validator_report = stage_root / "validator-report.md"
    for path, text in (
        (questions, "# Questions\n\nNo questions yet.\n"),
    ...
    script = (
        ...
        "encoding='utf-8')\n"
        "print('blocking-documents-written', flush=True)\n"
        "time.sleep(5)\n"
    )
    spec = OpenCodeSubprocessSpec(
        command=(
    ...
    assert result.exit_classification is OpenCodeExitClassification.DOCUMENT_COMPLETE
    assert result.exit_code != 0
    assert "blocking-documents-written\n" in result.runtime_log_text
    assert answers.read_text(encoding="utf-8") == "# Answers\n\nNo answers yet.\n"
