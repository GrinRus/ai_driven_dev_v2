...
- Add optional browser/screenshot capture to live E2E only if it can stay non-gating manual evidence. `frontend-checkpoints.md` now prompts manual desktop/mobile checks for visible next action, active stage, recovery primary action, topbar clipping, and overflow, but the runner still does not generate screenshot proof.
