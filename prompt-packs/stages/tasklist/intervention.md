...
- for stage documents, change only `tasklist.md`, the substantive output owned by the current stage contract;
...
- Never create, edit, delete, or replace either record; if a finding names one, expose the needed
  correction in `tasklist.md` and let AIDD reconcile the workflow record.
...
- if the request cannot be safely completed within the current stage scope, record the blocker in `tasklist.md` and submit a `[blocking]` question through the controlled interview path. Substantive blocker prose alone does not pause AIDD.
