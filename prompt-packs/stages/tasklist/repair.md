...
repair the runtime-authored `tasklist.md` only. Do not end the turn after analysis-only reads.
...
3. Keep dependency references resolvable to listed task ids or explicit upstream artifacts.
...
AIDD owns downstream-order drift correction: the immediate next stage after `tasklist` is
`implement`. Preserve content needed for that handoff; do not bypass the planning or
