...
- for stage documents, change only `review-spec-report.md`, the substantive output owned by the current stage contract;
...
- if the request cannot be safely completed within the current stage scope, record the blocker in `review-spec-report.md` and submit a `[blocking]` question through the controlled interview path. Substantive blocker prose alone does not pause AIDD.
