...
4. Write or update `stage-result.md` and `validator-report.md` so status and readiness state are consistent.
5. If plan artifacts or review context are missing or contradictory, raise a question instead of inventing assumptions.
6. Keep the output useful for downstream task decomposition rather than merely well-formatted.
