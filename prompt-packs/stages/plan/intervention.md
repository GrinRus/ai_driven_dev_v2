...
- for stage documents, change only `plan.md`, the substantive output owned by the current stage contract;
...
- if the request cannot be safely completed within the current stage scope, record the blocker in `plan.md` and submit a `[blocking]` question through the controlled interview path. Substantive blocker prose alone does not pause AIDD.
