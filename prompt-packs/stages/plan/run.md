...
- Runtime content must expose plan and evidence details for reconciliation but must not create or
...
8. Leave canonical `validator-report.md` and `stage-result.md` generation to AIDD; expose plan,
   dependency, and question evidence needed for reconciliation.
9. Keep operator questions and substantive plan content truthful; AIDD reconciles lifecycle status,
   validator verdict, blockers, and next actions.
