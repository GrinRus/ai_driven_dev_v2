...
from aidd.core.operator_frontend_dashboard_reducer import OperatorDashboardEvidence
...
def _stage_result_or_none(
    *,
    ...
    cache_key = ("stage-result", str(workspace_root), work_item, run_id, stage)
    if cache is not None and cache_key in cache:
        return cast(StageResultSummary | None, cache[cache_key])
    try:
...
def _questions_view(
    *,
    ...
    cache = _READ_CACHE.get()
    cache_key = ("questions", str(workspace_root), work_item, stage)
    if cache is not None and cache_key in cache:
        return cast(OperatorQuestionsView, cache[cache_key])
    result = resolve_operator_questions_view(
        workspace_root=workspace_root,
        work_item=work_item,
        stage=stage,
    )
...
def _collect_operator_dashboard_evidence(
    *,
    workspace_root: Path,
    work_item: str,
...
def collect_operator_dashboard_evidence(
    *,
    workspace_root: Path,
    work_item: str,
    active_stage: str = STAGES[0],
    ...
    try:
        return _collect_operator_dashboard_evidence(
            workspace_root=workspace_root,
            work_item=work_item,
            active_stage=active_stage,
            run_id=run_id,
