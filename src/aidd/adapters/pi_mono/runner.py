...
@dataclass(frozen=True, slots=True)
class PiMonoRunResult:
    exit_code: int
    stdout_text: str
    stderr_text: str
    runtime_log_text: str
    exit_classification: PiMonoExitClassification


class PiMonoExitClassification(StrEnum):
    SUCCESS = "success"
...
def _resolve_exit_classification(
    *,
    exit_code: int,
    stop_reason: PiMonoExitClassification | None,
) -> PiMonoExitClassification:
    if stop_reason is not None:
        return stop_reason
    if exit_code == 0:
        return PiMonoExitClassification.SUCCESS
    return PiMonoExitClassification.NON_ZERO_EXIT
...
def assemble_command(
    *,
    ...
    context: PiMonoCommandContext,
    repository_root: Path | None = None,
) -> tuple[str, ...]:
    stripped = configured_command.strip()
    if not stripped:
        raise ValueError("Configured pi-mono command must not be empty.")

    try:
        base_tokens = shlex.split(stripped)
    except ValueError as exc:
        raise ValueError(
            f"Configured pi-mono command is not valid shell syntax: {configured_command!r}"
        ) from exc
    if not base_tokens:
        raise ValueError("Configured pi-mono command must produce at least one token.")
...
def build_subprocess_spec(
    *,
    ...
    base_env: Mapping[str, str] | None = None,
    repository_root: Path | None = None,
) -> PiMonoSubprocessSpec:
    resolved_workspace_root = context.workspace_root.resolve(strict=False)
    return PiMonoSubprocessSpec(
...
def run_subprocess_with_streaming(
    *,
    ...
    stop_reason: PiMonoExitClassification | None = None
    completed_readers = 0
    while completed_readers < 2:
        if stop_reason is None:
            if cancel_requested is not None and cancel_requested():
                stop_reason = PiMonoExitClassification.CANCELLED
                _request_subprocess_stop(process)
            elif deadline is not None and time.monotonic() >= deadline:
                stop_reason = PiMonoExitClassification.TIMEOUT
                _request_subprocess_stop(process)
